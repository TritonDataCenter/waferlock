...
PgHbaPool.prototype.refreshTag = function (tag, addrs, cb) {
	...
	if (Object.keys(held).length > 0) {
		...
		for (var key in held) {
			mod_assert.strictEqual(self.php_timers[key], undefined);
			self.php_timers[key] = setTimeout(
			    self.expire.bind(self, key), timeout);
			self.php_held[key] = held[key];
		}

		self.php_log.info({ addrs: held, tag: tag }, 'holding %d ' +
		    'addresses for %d ms', Object.keys(held).length, timeout);
	}
